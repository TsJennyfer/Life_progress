...
app.get('/users/confirmEmail/:token', (req, res) => {
    ...
    User.findOneAndUpdate({ 'tokens.token': token }, { activated: true }).then((user) => {
        ...
        //res.send({ user });
        res.redirect('https://life-progress.herokuapp.com');
    }).catch((error) => {
...
app.get('/users/resetPasswordToken/:token', (req, res) => {
    ...
    User.findOne({ 'tokens.token': token }).then((user) => {
        ...
        res.redirect('https://life-progress.herokuapp.com'); //przekierowanie do formularza zmiany hasła
        res.send({ user });
    }).catch((error) => {
        res.status(400).send(error);
    });
...
// Edycja opisu celu
app.patch('/goals/updateDescription/:id', authenticate, (req, res) => {
    var id = req.params.id;
    var body = _.pick(req.body, ['description']); //jakie pola zmieniamy
